...
function change_status(client) {
    try {
        client.user.setActivity(`${config.prefix}help | ${config.prefix}setup`, {
            type: "WATCHING"
            // shardID: shard
        })
    } catch (e) {
...
function create_join_to_create_Channel(client, user, type) {
    ...
    if (!user.guild.me.permissions.has(Discord.Permissions.FLAGS.MANAGE_CHANNELS)) {
        allowed = false;
        try {
            user.member.user.send(`${user.member.user} I Do Not Have Permission To Create Channel. \`MANGE CHANNELS\` Is Required.`)
        } catch {
            try {
                ...
                channel.send(`${user.member.user} I Do Not Have Permission To Create Channel. \`MANGE CHANNELS\` Is Required.`)
            } catch { }
        }
    };
    if (allowed) {
